...
List ALL missing information that would help refine the assessment.`;
    ...
    try {
        ...
        fallback._aiActuallyUsed = false;
        fallback._scoringMethod = 'rule-based-fallback';
        fallback._devWarning = `AI_FAILED: Could not parse JSON response - ${content?.slice(0, 100)}`;
        return fallback;
    }
}
/**
 ...
 */
async function batchMatchPatientsToTrial(patients, criteria, options) {
    ...
    for (let i = 0; i < patients.length; i += maxConcurrent) {
        ...
        const batchResults = await Promise.all(batch.map((patient) => matchPatientToTrial(patient, criteria, parsedEligibility).catch((err) => {
...
// Helper Functions
// ============================================================================
function checkQuickExclusions(patient, criteria) {
    // Age check
...
function createExclusionResult(patient, reason) {
    return {
        patientId: patient.id,
        patientName: `${patient.firstName} ${patient.lastName}`,
        overallScore: 0,
...
${activeProblems
            .map((p) => `- ${p.description}${p.icd10_code ? ` [ICD-10: ${p.icd10_code}]` : ""}${p.onset_date ? ` (onset: ${p.onset_date})` : ""}`)
            .join("\n") || "None documented"}`);
...
${patient.allergies
            .map((a) => `- ${a.name}${a.reaction ? ` - Reaction: ${a.reaction}` : ""}${a.severity ? ` (${a.severity})` : ""}`)
            .join("\n")}`);
    }
    else {
...
${patient.labResults
            .map((l) => `- ${l.test_name}: ${l.value}${l.unit ? ` ${l.unit}` : ""}${l.abnormal ? " (ABNORMAL)" : ""}${l.reference_range ? ` [ref: ${l.reference_range}]` : ""}`)
            .join("\n")}`);
    }
 ...
 */
function ruleBasedMatching(patient, criteria) {
    const matchFactors = [];
    let totalScore = 0;
...
function checkGenderMatch(patient, criteria) {
    if (!criteria.gender || criteria.gender === "All") {
        return {
            category: "demographics",
...
function checkConditionMatch(patient, criteria) {
    ...
    for (const trialCond of trialConditions) {
        for (const patientCond of patientConditions) {
            if (patientCond.description.includes(trialCond) ||
...
function checkMedicationCompatibility(patient, criteria) {
    const activeMeds = patient.medications.filter((m) => m.status === "Active" || m.status === "active");
    ...
    const hasExclusionMed = activeMeds.some((m) => exclusionMeds.some((ex) => m.medication_name.toLowerCase().includes(ex)));
    return {
        category: "medications",
        name: "Medication Analysis",
...
function checkAllergyCompatibility(patient, criteria) {
    if (patient.allergies.length === 0) {
        return {
            category: "allergies",
            name: "Allergy Check",
    ...
    const severeAllergies = patient.allergies.filter((a) => a.severity?.toLowerCase() === "severe" || a.severity?.toLowerCase() === "life-threatening");
    return {
        category: "allergies",
        name: "Allergy Check",
        ...
        reason: severeAllergies.length > 0
...
function checkVitalsCompatibility(patient) {
    if (!patient.vitals) {
        return {
            category: "vitals",
            name: "Vital Signs",
...
function getMissingInformation(patient) {
    const missing = [];
    if (patient.problems.length === 0)
        missing.push("Medical history/conditions");
    if (patient.medications.length === 0)
        missing.push("Current medications");
    if (patient.allergies.length === 0)
        missing.push("Allergy information");
    if (!patient.vitals)
        missing.push("Recent vital signs");
    if (!patient.labResults || patient.labResults.length === 0)
        missing.push("Laboratory results");
    return missing;
}

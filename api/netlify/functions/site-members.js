...
const handler = async (event) => {
    ...
    try {
      ...
      `;
            ...
            // Get role counts
            const countResult = await (0, db_1.query)(`SELECT role, COUNT(*) as count
         FROM site_members
         WHERE site_id = $1 AND status = 'active'
         GROUP BY role`, [siteId]);
            const roleCounts = {};
        ...
        if (event.httpMethod === "POST") {
            ...
            const existing = await (0, db_1.query)(`SELECT id, status FROM site_members WHERE site_id = $1 AND email = $2`, [siteId, normalizedEmail]);
            if (existing.rows.length > 0) {
                ...
                await (0, db_1.query)(`UPDATE site_members
           SET role = $1, status = 'pending', invited_by = $2, invited_at = NOW()
           WHERE site_id = $3 AND email = $4`, [role, userId, siteId, normalizedEmail]);
            }
            else {
                // Create new invitation
                await (0, db_1.query)(`INSERT INTO site_members (site_id, email, role, status, invited_by)
           VALUES ($1, $2, $3, 'pending', $4)`, [siteId, normalizedEmail, role, userId]);
            }
            // Check if user already exists in system
            const userResult = await (0, db_1.query)(`SELECT user_id FROM users WHERE email = $1`, [normalizedEmail]);
            if (userResult.rows.length > 0) {
                // Link existing user
                await (0, db_1.query)(`UPDATE site_members SET user_id = $1 WHERE site_id = $2 AND email = $3`, [userResult.rows[0].user_id, siteId, normalizedEmail]);
            }
            // TODO: Send invitation email
            await (0, db_1.logAuditEvent)(userId, "MEMBER_INVITED", "site_member", normalizedEmail, undefined, { role, siteId }, event.headers?.["x-forwarded-for"], event.headers?.["user-agent"]);
        ...
        if (event.httpMethod === "PUT") {
            ...
            // Accept invitation
            if (action === "accept") {
                const result = await (0, db_1.query)(`UPDATE site_members
           SET status = 'active', user_id = $1, accepted_at = NOW()
           WHERE email = $2 AND status = 'pending'
           RETURNING site_id as "siteId", role`, [userId, body.email || event.headers["x-user-email"]]);
                if (result.rows.length === 0) {
                    return cors.response(404, { ok: false, error: "No pending invitation found" });
                }
                await (0, db_1.logAuditEvent)(userId, "INVITATION_ACCEPTED", "site_member", result.rows[0].siteId, undefined, { role: result.rows[0].role }, event.headers?.["x-forwarded-for"], event.headers?.["user-agent"]);
                return cors.response(200, {
                    ok: true,
           ...
           RETURNING id`, [role, siteId, email.trim().toLowerCase()]);
                if (result.rows.length === 0) {
                    return cors.response(404, { ok: false, error: "Member not found" });
                }
                await (0, db_1.logAuditEvent)(userId, "MEMBER_ROLE_UPDATED", "site_member", email, undefined, { newRole: role, siteId }, event.headers?.["x-forwarded-for"], event.headers?.["user-agent"]);
                return cors.response(200, {
                    ok: true,
        ...
        if (event.httpMethod === "DELETE") {
            ...
            const result = await (0, db_1.query)(`DELETE FROM site_members
         WHERE site_id = $1 AND email = $2
         RETURNING id`, [siteId, email.trim().toLowerCase()]);
            if (result.rows.length === 0) {
                return cors.response(404, { ok: false, error: "Member not found" });
            }
            await (0, db_1.logAuditEvent)(userId, "MEMBER_REMOVED", "site_member", email, undefined, { siteId }, event.headers?.["x-forwarded-for"], event.headers?.["user-agent"]);
            return cors.response(200, {
                ok: true,

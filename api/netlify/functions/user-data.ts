...
interface HealthProfile {
  ...
  diseaseStage?: string;
  biomarkers?: string;
  priorTherapies?: Array<{ name: string; date?: string }>;
...
async function ensureTablesExist(): Promise<void> {
  ...
  await query(`CREATE INDEX IF NOT EXISTS idx_eligibility_profiles_user_id ON eligibility_profiles(user_id)`);
}
...
async function getUserFromRequest(event: HandlerEvent): Promise<{ userId: string; email: string } | null> {
...
async function getHealthProfileData(userId: string): Promise<HealthProfile | null> {
  const result = await query(
...
async function getHealthProfile(userId: string) {
  const profile = await getHealthProfileData(userId);
  return cors.response(200, { profile });
}

async function saveHealthProfile(userId: string, profile: HealthProfile) {
  await query(
...
async function getPreferencesData(userId: string): Promise<UserPreferences | null> {
  const result = await query(
...
async function getEligibilityData(userId: string): Promise<EligibilityProfile | null> {
  const result = await query(
    `SELECT * FROM eligibility_profiles WHERE user_id = $1`,
    [userId]
  );
...
async function saveEligibility(userId: string, elig: EligibilityProfile) {
  await query(

    ...
    try {
      ...
      `;
        ...
        if (event.httpMethod === "POST") {
            ...
            const { patientId, nctId, title, description, appointmentType = "screening", startTime, endTime, location, videoLink, notes, } = body;
            ...
            const result = await (0, db_1.query)(`INSERT INTO appointments (
        ...
        RETURNING
          ...
          status`, [
            ...
            await (0, db_1.logAuditEvent)(userId, "APPOINTMENT_CREATED", "appointment", appointmentId, patientId, { title, startTime, nctId }, event.headers?.["x-forwarded-for"], event.headers?.["user-agent"]);
        ...
        if (event.httpMethod === "PUT") {
            ...
            const result = await (0, db_1.query)(`UPDATE appointments
         ...
         RETURNING
           ...
           status`, params);
            ...
            await (0, db_1.logAuditEvent)(userId, "APPOINTMENT_UPDATED", "appointment", appointmentId, body.patientId, { updatedFields: Object.keys(body) }, event.headers?.["x-forwarded-for"], event.headers?.["user-agent"]);
            return cors.response(200, {
                ok: true,
        ...
        if (event.httpMethod === "DELETE") {
         ...
         RETURNING id`, [appointmentId, providerId]);
            if (result.rows.length === 0) {
                return cors.response(404, { ok: false, error: "Appointment not found" });
            }
            await (0, db_1.logAuditEvent)(userId, "APPOINTMENT_CANCELLED", "appointment", appointmentId, undefined, {}, event.headers?.["x-forwarded-for"], event.headers?.["user-agent"]);
            return cors.response(200, {
                ok: true,

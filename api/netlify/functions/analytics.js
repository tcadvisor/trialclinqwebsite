...
const handler = async (event) => {
    ...
    try {
        ...
        if (type === "summary") {
            // Get trial counts
            const trialsResult = await (0, db_1.query)(`SELECT
          COUNT(*) as total,
         ...
         WHERE provider_id = $1`, [providerId]);
            ...
            const pipelineResult = await (0, db_1.query)(`SELECT status, COUNT(*) as count
         ...
         GROUP BY status`, [providerId]);
            ...
            const funnelResult = await (0, db_1.query)(`SELECT
         ...
         WHERE provider_id = $1`, [providerId]);
            ...
            try {
                ...
                const customDbResult = await (0, db_1.query)(`SELECT COUNT(*) as count, COALESCE(SUM(patient_count), 0) as total_patients
           FROM custom_patient_databases
           WHERE user_id = $1`, [userId]);
                // Custom matches by status
                const customMatchesResult = await (0, db_1.query)(`SELECT eligibility_status as status, COUNT(*) as count
           FROM custom_trial_matches
           WHERE user_id = $1
           GROUP BY eligibility_status`, [userId]);
            ...
            // Get appointments
            const appointmentsResult = await (0, db_1.query)(`SELECT
         ...
         FROM appointments
         WHERE provider_id = $1 AND status != 'cancelled'`, [providerId]);
            // Get recent activity (audit logs)
            const activityResult = await (0, db_1.query)(`SELECT action, resource_type, resource_id, created_at
         FROM audit_logs
         WHERE user_id = $1
         ...
         LIMIT 10`, [userId]);
            ...
            try {
                performanceResult = await (0, db_1.query)(`SELECT
            pt.nct_id as "nctId",
            pt.title,
        ...
        if (type === "trial-performance") {
            ...
            try {
                ...
                result = await (0, db_1.query)(`SELECT
            pt.nct_id as "nctId",
            pt.title,
            pt.status,
            pt.phase,
           ...
           ORDER BY "totalPatients" DESC`, [providerId]);
            }
            catch (e) {
                // Fallback without custom matches
                result = await (0, db_1.query)(`SELECT
        ...
        if (type === "export") {
           ...
           ORDER BY pp.created_at DESC`, [providerId]);
                data = result.rows;
            }
           ...
           FROM appointments
           WHERE provider_id = $1
           ORDER BY start_time DESC`, [providerId]);
                data = result.rows;
            }

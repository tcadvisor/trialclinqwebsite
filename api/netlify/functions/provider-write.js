...
const handler = async (event) => {
    ...
    try {
        ...
        const { providerId, email, emailVerified, siteName, organization, organizationType, organizationAbbreviation, parentOrganizations, address, city, state, zipcode, country, facilityType, fundingOrganization, acceptedConditions, languages, investigatorName, investigatorPhone, investigatorEmail, affiliatedOrganization, regulatoryAuthority, regulatoryAuthorityAddress, consentsAccepted, additionalInfo, } = payload;
        if (!providerId || !email) {
        ...
        if (authenticatedUser.role === 'provider' && authenticatedUser.userId !== providerId) {
            await (0, db_1.logAuditEvent)(authenticatedUser.userId, 'UNAUTHORIZED_ACCESS', 'provider_profile', providerId, providerId, { reason: 'User attempted to access another provider\'s profile' }, event.headers?.['x-forwarded-for'] || event.headers?.['x-client-ip'], event.headers?.['user-agent']);
      ...
      `, [
        ...
        // Log audit event
        await (0, db_1.logAuditEvent)(authenticatedUser.userId, 'PROVIDER_PROFILE_UPDATED', 'provider_profile', String(result.rows[0]?.id), providerId, { fields_updated: Object.keys(payload).length }, event.headers?.['x-forwarded-for'] || event.headers?.['x-client-ip'], event.headers?.['user-agent']);
            ...
            try {
                ...
                if (authHeader) {
                    ...
                    if (authenticatedUser) {
                        const payload = event.body ? JSON.parse(event.body) : {};
                        await (0, db_1.logAuditEvent)(authenticatedUser.userId, 'PROVIDER_PROFILE_UPDATE_FAILED', 'provider_profile', payload?.providerId, payload?.providerId, { error: e.message }, event.headers?.['x-forwarded-for'] || event.headers?.['x-client-ip'], event.headers?.['user-agent']);
                    }
                }
            }
            catch (_) {

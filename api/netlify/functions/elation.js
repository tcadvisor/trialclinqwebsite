...
// ============================================================================
async function ensureElationTables() {
...
async function getAccessToken(providerId) {
    const result = await (0, db_1.query)(`SELECT access_token, token_expires_at, refresh_token
...
async function refreshElationToken(providerId, refreshToken) {
    try {
        ...
        await (0, db_1.query)(`UPDATE elation_connections
       SET access_token = $1, refresh_token = $2, token_expires_at = $3, updated_at = NOW()
       WHERE provider_id = $4`, [tokens.access_token, tokens.refresh_token || refreshToken, expiresAt.toISOString(), providerId]);
...
async function fetchFromElation(accessToken, endpoint, params) {
    try {
...
const handler = async (event) => {
    ...
    try {
        ...
        if (event.httpMethod === "GET") {
            ...
            if (action === "status") {
                const result = await (0, db_1.query)(`SELECT practice_name, last_sync_at, connected_at
           FROM elation_connections
           WHERE provider_id = $1`, [providerId]);
                if (result.rows.length === 0) {
                ...
                const countResult = await (0, db_1.query)(`SELECT COUNT(*) as count FROM elation_patients WHERE provider_id = $1`, [providerId]);
                return cors.response(200, {
                    ok: true,
                    connected: true,
            ...
            if (action === "patient") {
                ...
                const result = await (0, db_1.query)(`SELECT * FROM elation_patients
           WHERE provider_id = $1 AND elation_patient_id = $2`, [providerId, parseInt(patientId, 10)]);
                if (result.rows.length === 0) {
        ...
        `;
            ...
            if (action === "matches") {
                ...
                const result = await (0, db_1.query)(`SELECT m.*, p.first_name, p.last_name, p.email, p.problems
           ...
           ORDER BY m.match_score DESC`, [providerId, nctId.toUpperCase()]);
                return cors.response(200, {
                    ok: true,
        ...
        if (event.httpMethod === "POST") {
            ...
            if (action === "connect_credentials") {
                ...
                try {
                    ...
                    const expiresAt = new Date(Date.now() + tokens.expires_in * 1000);
                    // Store connection
                    await (0, db_1.query)(`INSERT INTO elation_connections (provider_id, access_token, refresh_token, token_expires_at, connected_at)
             VALUES ($1, $2, $3, $4, NOW())
             ON CONFLICT (provider_id) DO UPDATE SET
               access_token = $2, refresh_token = $3, token_expires_at = $4, connected_at = NOW(), updated_at = NOW()`, [providerId, tokens.access_token, tokens.refresh_token || null, expiresAt.toISOString()]);
                    await (0, db_1.logAuditEvent)(userId, "ELATION_CONNECTED", "elation_connection", providerId, undefined, { method: "client_credentials" }, event.headers?.["x-forwarded-for"], event.headers?.["user-agent"]);
                    return cors.response(200, {
                        ok: true,
            ...
            if (action === "initiate_oauth") {
                ...
                await (0, db_1.query)(`INSERT INTO elation_connections (provider_id, access_token)
           VALUES ($1, $2)
           ON CONFLICT (provider_id) DO UPDATE SET updated_at = NOW()`, [providerId, `state:${state}`]);
           ...
           WHERE provider_id = $4`, [tokens.access_token, tokens.refresh_token, expiresAt.toISOString(), providerId]);
                await (0, db_1.logAuditEvent)(userId, "ELATION_CONNECTED", "elation_connection", providerId, undefined, {}, event.headers?.["x-forwarded-for"], event.headers?.["user-agent"]);
            ...
            if (action === "sync_patients") {
                ...
                const patientsResponse = await fetchFromElation(accessToken, "/patients/", { limit: limit.toString(), offset: offset.toString() });
                if (!patientsResponse) {
                ...
                for (const patient of patients) {
                    try {
                        ...
                        const [problems, allergies, medications] = await Promise.all([
                            fetchFromElation(accessToken, "/problems/", {
                                patient: patient.id.toString(),
                            }),
                            fetchFromElation(accessToken, "/allergies/", {
                                patient: patient.id.toString(),
                            }),
                            fetchFromElation(accessToken, "/medications/", {
                                patient: patient.id.toString(),
                            }),
                        ]);
                        // Filter by condition if specified
                        if (problemFilter) {
                            const hasCondition = problems?.results?.some((p) => p.description?.toLowerCase().includes(problemFilter.toLowerCase()) ||
                                p.icd10_code?.toLowerCase().includes(problemFilter.toLowerCase()));
                            if (!hasCondition)
                                continue;
                        }
                        // Upsert patient
                        await (0, db_1.query)(`INSERT INTO elation_patients (
              ...
              ON CONFLICT (provider_id, elation_patient_id) DO UPDATE SET
                ...
                await (0, db_1.query)(`UPDATE elation_connections SET last_sync_at = NOW() WHERE provider_id = $1`, [providerId]);
                await (0, db_1.logAuditEvent)(userId, "ELATION_SYNC", "elation_patients", providerId, undefined, { syncedCount, totalAvailable: patientsResponse.count }, event.headers?.["x-forwarded-for"], event.headers?.["user-agent"]);
                return cors.response(200, {
                    ok: true,
                    syncedCount,
...
ACTIVE MEDICAL CONDITIONS (${problems.filter((p) => p.status === 'Active').length}):
${problems.filter((p) => p.status === 'Active').map((p) => `- ${p.description}${p.icd10_code ? ` [ICD-10: ${p.icd10_code}]` : ''}${p.onset_date ? ` (onset: ${p.onset_date})` : ''}`).join('\n') || '- None documented'}

CURRENT MEDICATIONS (${medications.filter((m) => m.status === 'Active').length}):
${medications.filter((m) => m.status === 'Active').map((m) => `- ${m.medication_name}${m.dosage ? ` ${m.dosage}` : ''}${m.frequency ? ` (${m.frequency})` : ''}`).join('\n') || '- None documented'}

ALLERGIES (${allergies.length}):
${allergies.map((a) => `- ${a.name}${a.reaction ? ` - Reaction: ${a.reaction}` : ''}${a.severity ? ` (Severity: ${a.severity})` : ''}`).join('\n') || '- No known allergies (NKDA)'}
...
TARGET CONDITIONS:
${trialCtx.conditions.map((c) => `- ${c}`).join('\n') || '- Not specified'}

INCLUSION CRITERIA:
${trialCtx.inclusionCriteria.map((c) => `- ${c}`).join('\n') || '- Not specified'}

EXCLUSION CRITERIA:
${trialCtx.exclusionCriteria.map((c) => `- ${c}`).join('\n') || '- Not specified'}
...
1. Review EVERY condition listed above (there are ${problems.filter((p) => p.status === 'Active').length} active conditions) - not just the first one
2. Check EVERY medication (there are ${medications.filter((m) => m.status === 'Active').length} active medications) for interactions
...
Analyze the patient's eligibility comprehensively. Check each inclusion criterion and exclusion criterion. Consider the COMPLETE medical history, ALL current medications, ALL allergies, and vital signs. Return your assessment as JSON with eligibilityScore being the calculated SUM.`;
                ...
                function ruleBasedScoring(patient, trialCtx) {
                    const age = patient.dob
                        ? Math.floor((Date.now() - new Date(patient.dob).getTime()) / (365.25 * 24 * 60 * 60 * 1000))
                        : null;
                    const problems = patient.problems || [];
                    ...
                    const normalizeCondition = (s) => s.toLowerCase().replace(/[^a-z0-9]/g, ' ').trim();
                    const getConditionTokens = (s) => normalizeCondition(s).split(/\s+/).filter(t => t.length > 2);
                    const calculateConditionSimilarity = (patientCond, trialCond) => {
                        const patientTokens = getConditionTokens(patientCond);
                        const trialTokens = getConditionTokens(trialCond);
                        if (patientTokens.length === 0 || trialTokens.length === 0)
                            return 0;
                    ...
                    const icd10FamilyMatch = (patientCode, trialCondition) => {
                        if (!patientCode)
                            return false;
                    ...
                    const conditionScoreDetails = [];
                    if (trialCtx.conditions && trialCtx.conditions.length > 0) {
                        const conditionTerms = trialCtx.conditions;
                    ...
                    if (activeProblems.length === 0)
                        missingInfo.push("No conditions documented");
                    if (activeMeds.length === 0)
                        missingInfo.push("No medications documented");
                    if (!hasVitalsData)
                        missingInfo.push("No vital signs");
                    return {
                        patientId: patient.elation_patient_id,
                        firstName: patient.first_name,
                        lastName: patient.last_name,
                ...
                for (let i = 0; i < patients.length; i += BATCH_SIZE) {
                    ...
                    for (const result of batchResults) {
                        if (!result)
                            continue;
                        if (result.eligibilityScore < 25)
                            continue;
                        // Track common matching conditions
                        if (result.matchingConditions) {
                            result.matchingConditions.forEach((cond) => {
              ...
              ON CONFLICT (provider_id, elation_patient_id, nct_id) DO UPDATE SET
                ...
                await (0, db_1.logAuditEvent)(userId, "TRIAL_MATCHING_COMPLETED", "elation_trial_matches", nctId, undefined, { matchCount, aiPowered: aiAnalysisUsed, patientsAnalyzed: patients.length, aiSuccessCount, aiFallbackCount }, event.headers?.["x-forwarded-for"], event.headers?.["user-agent"]);
                return cors.response(200, {
                    ok: true,
                    matchCount,
                    nctId,
            ...
            if (action === "batch_accept_matches") {
                ...
                const matchesResult = await (0, db_1.query)(`SELECT m.elation_patient_id, m.match_score, p.first_name, p.last_name, p.email,
           ...
           AND m.eligibility_status IN ('potential', 'pending')`, [providerId, nctId.toUpperCase(), threshold]);
                // Update match statuses
                const updateResult = await (0, db_1.query)(`UPDATE elation_trial_matches
           ...
           RETURNING elation_patient_id`, [newStatus, providerId, nctId.toUpperCase(), threshold]);
                ...
                if (addToPipeline && matchesResult.rows.length > 0) {
                    for (const match of matchesResult.rows) {
                        try {
                            ...
                            await (0, db_1.query)(`INSERT INTO patient_profiles (patient_id, email, primary_condition, created_at)
                 VALUES ($1, $2, $3, NOW())
                 ON CONFLICT (patient_id) DO UPDATE SET
                   email = COALESCE($2, patient_profiles.email),
                   primary_condition = COALESCE($3, patient_profiles.primary_condition),
                   updated_at = NOW()`, [patientId, match.email, match.primary_condition]);
                            // Add to pipeline
                            await (0, db_1.query)(`INSERT INTO patient_pipeline (
                ...
                await (0, db_1.logAuditEvent)(userId, "BATCH_ACCEPT_MATCHES", "elation_trial_matches", nctId, undefined, { threshold, newStatus, matchCount: updateResult.rowCount, pipelineCount }, event.headers?.["x-forwarded-for"], event.headers?.["user-agent"]);
                return cors.response(200, {
                    ok: true,
            ...
            if (action === "add_to_pipeline") {
                ...
                const patientResult = await (0, db_1.query)(`SELECT p.*, m.match_score
           ...
           WHERE p.provider_id = $1 AND p.elation_patient_id = $2`, [providerId, elationPatientId, nctId.toUpperCase()]);
           ...
           ON CONFLICT (patient_id) DO UPDATE SET
             email = COALESCE($2, patient_profiles.email),
             primary_condition = COALESCE($3, patient_profiles.primary_condition),
             updated_at = NOW()`, [patientId, patient.email, primaryCondition]);
                // Add to pipeline
          ...
          ON CONFLICT (provider_id, patient_id, nct_id) DO UPDATE SET
            ...
            updated_at = NOW()
          RETURNING id, status`, [providerId, patientId, nctId.toUpperCase(), patient.match_score, notes || `Added from Elation EHR (${patient.first_name} ${patient.last_name})`]);
                // Update match status to indicate added to pipeline
                await (0, db_1.query)(`UPDATE elation_trial_matches
           SET eligibility_status = 'eligible', updated_at = NOW()
           WHERE provider_id = $1 AND elation_patient_id = $2 AND nct_id = $3`, [providerId, elationPatientId, nctId.toUpperCase()]);
                await (0, db_1.logAuditEvent)(userId, "ELATION_PATIENT_ADDED_TO_PIPELINE", "patient_pipeline", patientId, nctId, { elationPatientId, matchScore: patient.match_score }, event.headers?.["x-forwarded-for"], event.headers?.["user-agent"]);
                return cors.response(200, {
                    ok: true,
           ...
           WHERE provider_id = $2 AND elation_patient_id = $3 AND nct_id = $4`, [status, providerId, elationPatientId, nctId.toUpperCase()]);
                return cors.response(200, { ok: true });
            }
            // Disconnect Elation
            if (action === "disconnect") {
                await (0, db_1.query)(`DELETE FROM elation_trial_matches WHERE provider_id = $1`, [providerId]);
                await (0, db_1.query)(`DELETE FROM elation_patients WHERE provider_id = $1`, [providerId]);
                await (0, db_1.query)(`DELETE FROM elation_connections WHERE provider_id = $1`, [providerId]);
                await (0, db_1.logAuditEvent)(userId, "ELATION_DISCONNECTED", "elation_connection", providerId, undefined, {}, event.headers?.["x-forwarded-for"], event.headers?.["user-agent"]);

...
// Get or create user
async function getOrCreateUser(userId, email, azureOid, firstName, lastName, role = 'patient') {
    try {
...
// Audit log helper
async function logAuditEvent(userId, action, resourceType, resourceId, patientId, details, ipAddress, userAgent) {
    try {
        await query(`

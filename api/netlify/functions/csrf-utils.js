...
exports.getCsrfTokenFromHeaders = getCsrfTokenFromHeaders;
exports.corsWithCsrf = corsWithCsrf;
...
function generateCsrfToken() {
    const randomToken = (0, crypto_1.randomBytes)(TOKEN_BYTE_LENGTH).toString("base64url");
    const timestamp = Date.now();
    const payload = `${randomToken}.${timestamp}`;
    // Sign the payload with HMAC
    const signature = (0, crypto_1.createHmac)(HMAC_ALGORITHM, CSRF_SECRET)
        .update(payload)
        .digest("base64url");
 ...
 */
function verifySignature(token, signature) {
    try {
        const expectedSignature = (0, crypto_1.createHmac)(HMAC_ALGORITHM, CSRF_SECRET)
            .update(token)
            .digest("base64url");
        ...
        return (0, crypto_1.timingSafeEqual)(tokenBuffer, expectedBuffer);
    }
    catch {
        return false;
    }
 ...
 */
function corsWithCsrf(statusCode, body, additionalHeaders = {}) {
 ...
 */
exports.verifyCsrfToken = validateCsrfToken;

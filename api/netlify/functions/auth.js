...
function generateId(prefix) {
    return `${prefix}_${Date.now().toString(36)}_${crypto_1.default.randomBytes(12).toString("hex")}`;
}
...
const handler = async (event) => {
    ...
    try {
        ...
        if (action === "signup") {
            ...
            const existing = await (0, db_1.query)("SELECT id FROM auth_credentials WHERE email = $1", [normalizedEmail]);
            ...
            await (0, db_1.query)(`INSERT INTO auth_credentials (
          user_id, email, password_hash, email_verification_token, email_verification_expires
        ) VALUES ($1, $2, $3, $4, $5)`, [userId, normalizedEmail, passwordHash, verificationToken, verificationExpires]);
            // Create user record
            await (0, db_1.getOrCreateUser)(userId, normalizedEmail, "", firstName, lastName, role);
            await (0, db_1.logAuditEvent)(userId, "USER_SIGNUP", "user", userId, undefined, { email: normalizedEmail, role }, event.headers?.["x-forwarded-for"], event.headers?.["user-agent"]);
            // TODO: Send verification email
            // For now, auto-verify in development
            if (process.env.NODE_ENV !== "production") {
                await (0, db_1.query)(`UPDATE auth_credentials SET email_verified = true WHERE user_id = $1`, [userId]);
            }
        ...
        if (action === "signin") {
         ...
         WHERE ac.email = $1`, [normalizedEmail]);
            ...
            if (!passwordValid) {
                ...
                await (0, db_1.query)(`UPDATE auth_credentials
           ...
           WHERE user_id = $3`, [
                ...
                await (0, db_1.logAuditEvent)(user.user_id, "LOGIN_FAILED", "auth", user.user_id, undefined, { attempts: newAttempts }, event.headers?.["x-forwarded-for"], event.headers?.["user-agent"]);
                return cors.response(401, {
                    ok: false,
                    error: "Invalid email or password",
         ...
         WHERE user_id = $1`, [user.user_id]);
            await (0, db_1.query)(`INSERT INTO sessions (session_id, user_id, token_hash, expires_at, ip_address, user_agent)
         VALUES ($1, $2, $3, $4, $5, $6)`, [
                sessionId,
                user.user_id,
                tokenHash,
            ...
            if (role && role !== user.role) {
                await (0, db_1.query)(`UPDATE users SET role = $1 WHERE user_id = $2`, [role, user.user_id]);
            }
            await (0, db_1.logAuditEvent)(user.user_id, "LOGIN_SUCCESS", "auth", user.user_id, undefined, { sessionId }, event.headers?.["x-forwarded-for"], event.headers?.["user-agent"]);
        ...
        if (action === "signout") {
            ...
            if (token) {
                const tokenHash = hashToken(token);
                await (0, db_1.query)(`DELETE FROM sessions WHERE token_hash = $1`, [tokenHash]);
            }
            // Clear the cookie
            return {
                statusCode: 200,
        ...
        if (action === "validate") {
         ...
         WHERE s.token_hash = $1 AND s.expires_at > NOW()`, [tokenHash]);
            ...
            await (0, db_1.query)(`UPDATE sessions SET last_accessed = NOW() WHERE token_hash = $1`, [tokenHash]);
        ...
        if (action === "request-reset") {
         ...
         WHERE email = $3`, [resetToken, resetExpires, normalizedEmail]);
        ...
        if (action === "reset-password") {
            ...
            const result = await (0, db_1.query)(`SELECT user_id FROM auth_credentials
         WHERE password_reset_token = $1 AND password_reset_expires > NOW()`, [token]);
            if (result.rows.length === 0) {
                return cors.response(400, {
                    ok: false,
         ...
         WHERE user_id = $2`, [passwordHash, userId]);
            ...
            await (0, db_1.logAuditEvent)(userId, "PASSWORD_RESET", "auth", userId, undefined, {}, event.headers?.["x-forwarded-for"], event.headers?.["user-agent"]);
            return cors.response(200, {
                ok: true,
        ...
        if (action === "verify-email") {
            ...
            const result = await (0, db_1.query)(`UPDATE auth_credentials
